...
function nowTs() {
  const d = new Date();
  return chalk.gray(d.toTimeString().slice(0, 8)); // HH:MM:SS
}
...
export function createLogger({ level = 'info', name = 'scraper' } = {}) {
  ...
  const api = {
    ...
    // Enhanced progress bar with custom tokens
    progress(total, { label = 'Progress' } = {}) {
      const fmt =
        ...
        `${chalk.gray('|')} page:{pageCoords} geo:{geocoded} err:{errors}`;

      if (!can('info') || !isTTY) {
        let value = 0;
        let payload = { pageCoords: 0, geocoded: 0, errors: 0 };
        return {
          increment(step = 1, p = {}) {
            value += step;
            Object.assign(payload, p);
            line(ICONS.info, `${label}: ${value}/${total} (${Math.floor((value / total) * 100)}%) page:${payload.pageCoords} geo:${payload.geocoded} err:${payload.errors}`);
          },
          update(v, p = {}) {
            value = v;
            Object.assign(payload, p);
      ...
      bar.start(total, 0, { pageCoords: 0, geocoded: 0, errors: 0 });
